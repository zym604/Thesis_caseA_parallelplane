...
plt.savefig("step1_fig2", bbox_inches='tight')
#plot 2
plt.figure(figsize=(10,7))
s = 3
for i in range(t_d_oup.shape[1]):
...
for handle in lgnd.legendHandles:
    handle.set_sizes([30.0])
plt.savefig("step1_fig3", bbox_inches='tight')
...
AutoNetRegression().print_help()
autonet = AutoNetRegression(budget_type='epochs', 
                            #min_budget=100, 
                            #max_budget=900, 
                            #num_iterations=1, 
                            ...
                            optimizer=['adam'],
                            log_level='info')
results_fit = autonet.fit(X_train=t_d_inp,Y_train=t_d_oup,refit=True)
# Save json
import json
with open("step1_results_fit.json", "w") as file:
...
plt.savefig("step1_fig4", bbox_inches='tight')
#plot 2
plt.figure(figsize=(10,7))
s = 3
for i in range(t_d_oup.shape[1]):
...
for handle in lgnd.legendHandles:
    handle.set_sizes([30.0])
plt.savefig("step1_fig5", bbox_inches='tight')
